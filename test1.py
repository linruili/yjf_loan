...
def f5(arr):
    ...
    arr['withhold_send_time'] = arr['withhold_send_time'].apply(lambda x: datetime.datetime.strptime(x, "%Y-%m-%d %H:%M:%S")).sort_values()
    flag = False
...
all = all.drop(['installment_repay_type','order_period','total_times','order_amount'], axis=1)
